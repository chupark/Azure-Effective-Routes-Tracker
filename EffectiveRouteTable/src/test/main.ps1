...
$user = $loginConfig.servicePrincipal.user
$pass = ConvertTo-SecureString -String $loginConfig.servicePrincipal.pass -AsPlainText -Force
$cred = New-Object -TypeName System.Management.Automation.PsCredential -ArgumentList $user, $pass
$account = Connect-AzAccount -ServicePrincipal -Credential $cred -Tenant $loginConfig.accountBasic.tenant -Subscription $loginConfig.accountBasic.subscription -SkipContextPopulation -WarningAction SilentlyContinue
...
try {
    ...
    $csvFileName = $env:effectiveRoute + "EffectiveRouteTable\outputs\routeTable\" + $todayFile + "_route.csv"
